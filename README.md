...
$$
\tau_k = \operatorname{median}(E_k) + 1.4826\,\operatorname{MAD}(E_k).
$$
...
During replay, the offline tier is selected with probability \(p_{\mathrm{off}}=0.8\), and the online tier is selected with probability \(0.2\) when both tiers are nonempty. This corresponds to an expected offline-to-online replay ratio of \(4{:}1\).
...
| Offline replay probability \(p_{\mathrm{off}}\) | 0.8 |
| Batch size | 32 |
...
$$
AF_{\mathrm{base}} = \operatorname{RMSE}_{\mathrm{base}}^{f}
- \operatorname{RMSE}_{\mathrm{base}}^{0},
$$

where \(\operatorname{RMSE}_{\mathrm{base}}^{0}\) is the offline model's base-period RMSE and \(\operatorname{RMSE}_{\mathrm{base}}^{f}\) is the final adapted model's RMSE on the same base evaluation data.

The principal stability-plasticity score is

$$
SP = \operatorname{RMSE}_{\mathrm{stream}} + AF_{\mathrm{base}}.
$$

Lower \(SP\) indicates a better balance between online adaptation and base-period retention. Positive \(AF_{\mathrm{base}}\) indicates forgetting, whereas negative \(AF_{\mathrm{base}}\) indicates improved performance on the base evaluation data after adaptation.
...
- stream RMSE;
- base-retention forgetting \(AF_{\mathrm{base}}\);
